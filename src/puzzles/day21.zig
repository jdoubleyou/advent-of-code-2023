...
test "day 21, part 1" {
    ...
    try utils.write("day 21, part 1", response);
}

test "day 21, part 2 [example]" {
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    const allocator = arena.allocator();
    defer arena.deinit();
...
test "day 21, part 2" {
    ...
    try utils.write("day 21, part 2", response);
}
